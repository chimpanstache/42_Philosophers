...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/26 13:27:39 by ehafidi           #+#    #+#             */
/*   Updated: 2021/03/01 13:16:13 by ehafidi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	malloc_values(t_params *p)
{
	...
	if (!(p->last_meal_time = malloc(sizeof(long *) * p->nop)))
		return (1);
	if (!(p->mutexphilo = malloc(sizeof(pthread_mutex_t) * p->nop)))
		return (1);
	if (!(p->mutexindex = malloc(sizeof(pthread_mutex_t))))
		return (1);
	if (!(p->mutexindexcaner = malloc(sizeof(pthread_mutex_t))))
		return (1);
	if (!(p->philo = malloc(sizeof(pthread_t) * p->nop)))
		return (1);
	if (!(p->check_death = malloc(sizeof(pthread_t) * p->nop)))
		return (1);
	if (!(p->mutexoutput = malloc(sizeof(pthread_mutex_t))))
		return (1);
	if (!(p->mutexdie = malloc(sizeof(pthread_mutex_t))))
		return (1);
	return (0);
}
